...
module.exports = conn => {
   ...
   conn.fakeReply = async (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
      const { areJidsSameUser } = await getBaileys()
   ...
   conn.sendFDoc = async (jid, text, quoted, opts = {}) => {
      ...
      return conn.sendMessage(jid, {
         ...
         mimetype: (opts && opts.mime) ? opts.mime : Func.random(['application/vnd.openxmlformats-officedocument.presentationml.presentation', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/pdf', 'text/rtf']),
   ...
   conn.sendAlbumMessageV2 = async (jid, medias, options = {}) => {
      const { generateWAMessageFromContent, generateWAMessage, isJidNewsletter } = await getBaileys()
