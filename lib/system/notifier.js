      ...
      try {
         ...
         for (const [jid, group] of rentedGroups) {
            ...
            if (daysLeft > 0 && daysLeft <= 3 && group.lastnotified !== daysLeft) {
               const meta = await this.conn.getGroupMetadata(jid)
               if (data.setting.notifier) this.conn.reply(jid, Func.texted('italic', `🚩 Bot's active period for this group will expire in ${daysLeft} day(s).`), null, {
                  mentions: meta.participants.filter(v => v && v.phoneNumber).map(v => v.phoneNumber)
               ...
               await this.conn.groupLeave(jid)
