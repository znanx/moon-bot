...
module.exports = (conn, system) => {
   ...
   conn.on('group-participants.update', async x => {
      try {
         ...
         let pic
         try {
            pic = await conn.sock.profilePictureUrl(x.member, 'image')
         } catch {
            try {
               pic = await conn.sock.profilePictureUrl(x.jid, 'image')
            } catch {
               try {
                  pic = await Func.fetchBuffer('./lib/assets/images/default.jpg')
               } catch {
