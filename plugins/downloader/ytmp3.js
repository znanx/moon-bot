   ...
   run: async (m, {
      ...
      try {
         ...
         const format = data.formats?.find(
            item => item.extension === 'mp3' && item.quality === '128kbps'
         )

         if (!format?.task_token) {
            throw '128kbps MP3 format is unavailable.'
         }
         ...
         await conn.sendLinkPreview(m.chat, caption, m, {
            ratio: 'landscape',
         ...
         await conn.sendFile(
            m.chat,
            result.url,
            result.filename || `${data.title}.mp3`,
            '',
            m,
            {
               document: true,
